...
The pacakge is publicly available and can be installed from Bioconductor version 2.8 or higher in R version 2.13.0 or higher. 
...
\begin{description}
\item breastCancerMAINZ:  http://www.bioconductor.org/packages/release/data/experiment/html/breastCancerMAINZ.html
\item breastCancerUPP:  http://www.bioconductor.org/packages/release/data/experiment/html/breastCancerUPP.html
\item breastCancerUNT:  http://www.bioconductor.org/packages/release/data/experiment/html/breastCancerUNT.html
\item breastCancerNKI:  http://www.bioconductor.org/packages/release/data/experiment/html/breastCancerNKI.html
\item breastCancerTRANSBIG:http://www.bioconductor.org/packages/release/data/experiment/html/breastCancerTRANSBIG.html
\end{description}
...
`datasetMerging` <- 
  function (esets, nthread=1) 
    {

    ## all unique Entrez gene ids
    ## gene ids
    ugid <- lapply(esets, function(x) { return(Biobase::fData(x)[, is.element(colnames(fData(x)), c("probe", "EntrezGene.ID"))]) })
...
for (eset in esets){
    fData(eset)$EntrezGene.ID[fData(eset)$EntrezGene.ID == "" |fData(eset)$EntrezGene.ID == " " ] <- NA
    ## take out NA from fData, select those rows from expr
    eset.fdata <- fData(eset)[!is.na(fData(eset)$EntrezGene.ID),]
    eset.expr <- exprs(eset)[rownames(eset.fdata),]
  
  x <- cbind(eset.fdata$EntrezGene.ID,eset.expr)
  collapsed.wgcna.mrv <- WGCNA::collapseRows(x[,-1], rowID=rownames(x), rowGroup=x[,1], method="maxRowVariance")
  
  best_probe <- collapsed.wgcna.mrv[[3]]
  Biobase::exprs(eset) <- eset.expr[best_probe, ]
  Biobase::fData(eset) <- eset.fdata[best_probe,]
  rownames(Biobase::exprs(eset)) <- Biobase::fData(eset)$EntrezGene.ID
  rownames(Biobase::fData(eset)) <- Biobase::fData(eset)$EntrezGene.ID
  newEsets <- c(newEsets,eset)
}
...
data("pam50.scale")
  ...
  # scmod2.robust: List of parameters defining the subtype clustering model (as defined by Wirapati et al)
  SubtypePredictions<-subtype.cluster.predict(sbt.model=scmod2.robust,data=ddata,annot=dannot,do.mapping=TRUE,verbose=TRUE)
  #genefu::subtype.cluster.predict(sbt.model=genefu::scmod2.robust, data=datage, annot=annotge, do.mapping=TRUE)[c("subtype2", "subtype.proba2")] 
  #Get sample counter pertaining to each subtype
  ...
  #Select samples pertaining to Luminal Subtype
  Luminals<-names(which(SubtypePredictions$subtype == "ER+/HER2-"))
  #ASSIGN SUBTYPES TO EVERY SAMPLE, ADD TO THE EXISTING PHENODATA
  ddemo$SCMGENE<-SubtypePredictions$subtype
  ddemo[Luminals,]$SCMGENE<-"Luminal"
  ...
  #PAM50Preds<-intrinsic.cluster.predict(sbt.model=pam50,data=data.nkis,annot=annot.nkis,do.mapping=TRUE,verbose=TRUE)
  PAM50Preds<-intrinsic.cluster.predict(sbt.model=pam50,data=ddata,annot=dannot,do.mapping=TRUE,verbose=TRUE)
  table(PAM50Preds$subtype)
  ddemo$PAM50<-PAM50Preds$subtype
  LumA<-names(which(PAM50Preds$subtype == "LumA"))
  LumB<-names(which(PAM50Preds$subtype == "LumB"))
  Normals<-names(which(PAM50Preds$subtype=="Normal"))
  ddemo[LumA,]$PAM50<-"Luminal"
  ddemo[LumB,]$PAM50<-"Luminal"
@
...
confusionMatrix(ddemo[-which(rownames(ddemo) %in% Normals),]$SCMGENE,ddemo[-which(rownames(ddemo) %in% Normals),]$PAM50)
@

From these results, the concordance of the predictions between these models is ~83 percent. 
...
data.for.survival.SCMGENE <- ddemo[,c("e.os", "t.os", "SCMGENE")]
data.for.survival.PAM50 <- ddemo[,c("e.os", "t.os", "PAM50")]
...
surv.obj.SCMGENE <- survfit(Surv(data.for.survival.SCMGENE$months_to_death, data.for.survival.SCMGENE$vital_status) ~ data.for.survival.SCMGENE$SCMGENE)
...
for(i in 1:length(dn)) {
  ...
  ## GENE70
  # Need to do mapping for Affy platforms because this is based on Agilent. Hence the mapping rule is reversed here! 
...
# In the previous vignette for genefu version1, we were only testing 4 risk predictors, so we had a total of 722 complete cases remaining
...
To compare the risk score performances, we compute the concordance index\footnote{The same analysis could be performed with D index and hazard ratio by using the functions \Rfunction{D.index} and \Rfunction{hazard.ratio} from the \Rpackage{survcomp} package respectively}:
